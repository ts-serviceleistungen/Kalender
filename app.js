...
async function generateShiftPlan(){
  ...
  if(!startDate){$("shiftMsg").textContent="Bitte den ersten Arbeitstag auswählen.";return}
  const {data:{session}}=await db.auth.getSession();
  if(!session){$("shiftMsg").textContent="Bitte zuerst anmelden.";return}
  ...
  // Remove previously generated shift-plan events in this generated period.
  const del=await db.from("personal_calendar_events")
    .delete()
    .eq("source","shift_plan")
    .gte("shift_date",startDate)
    .lt("shift_date",isoDateLocal(until));
  if(del.error){$("shiftMsg").textContent="Fehler beim Bereinigen: "+del.error.message;return}

  const rows=[];
  for(let d=new Date(start);d<until;d.setDate(d.getDate()+1)){
    const dateStr=isoDateLocal(d);
    const ex=exceptionFor(dateStr);
    let isWork=shiftCycleIsWork(Math.round((d-start)/86400000));
    if(ex?.type==="Urlaub"||ex?.type==="Frei") isWork=false;
    if(ex?.type==="Arbeit") isWork=true;
    if(!isWork) continue;

    const s=new Date(`${dateStr}T${startTime}`);
    const eDate=new Date(d); eDate.setDate(eDate.getDate()+1);
  ...
  if(rows.length){
    ...
    if(ins.error){$("shiftMsg").textContent="Fehler beim Erstellen: "+ins.error.message;return}
  }

  // Additional work days are generated above; vacation/free days create no event,
  // therefore those days remain available for customer bookings.
  $("shiftMsg").textContent=`Fertig: ${rows.length} Arbeitsschichten erstellt. Urlaub/Frei bleibt für Kunden buchbar.`;
  await load();
