...
function renderTimeView(){
  ...
  const a=isDay?new Date(current):startOfWeek(current);
  const days=isDay?1:7;
  let html='<div class="week">';

  // Kopfzeile: in der Wochenansicht sieben echte Tages-Spalten.
  html+='<div class="timeHeader"><div class="timeHeaderEmpty"></div>';
  for(let i=0;i<days;i++){
    const d=new Date(a);
    d.setDate(d.getDate()+i);
    html+=`<div class="dayHeader">${d.toLocaleDateString("de-DE",{weekday:"short"})}<br><b>${pad(d.getDate())}.${pad(d.getMonth()+1)}.</b></div>`;
  }
  html+='</div>';

  for(let h=0;h<24;h++){
    html+=`<div class="timeRow ${isDay?'singleDay':''}"><div class="time">${pad(h)}:00</div>`;
    for(let i=0;i<days;i++){
      const d=new Date(a);
      d.setDate(d.getDate()+i);
      const s=new Date(d); s.setHours(h,0,0,0);
      const en=new Date(s); en.setHours(h+1);
      html+='<div class="slot">';
      events
        .filter(e=>{
          const es=new Date(e.start_time);
          const ee=new Date(e.end_time);
          // Ein Termin wird nur in seiner tatsächlichen Start-Stunde angezeigt.
          // Besonders wichtig für 24-Stunden-Schichten: keine Duplikate in jeder Stunde.
          return es>=s && es<en && ee>s;
        })
